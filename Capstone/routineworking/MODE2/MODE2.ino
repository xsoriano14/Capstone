...
Serial.println("BEFORE numruns: " + String(numruns) + "");
                ...
                if(numruns >= birdieCountGlobal){
                  numruns = 0;
                  client.println("<script>setMode2Running(false);</script>");
...
client.println("  }");
client.println("  function delayjava(ms) {");
client.println("    return new Promise(resolve => setTimeout(resolve, ms));");
client.println("  }");
client.println("  delayjava(10000).then(() => {");  // 10-second initial delay
client.println("    // schedule each shot");
client.println("    for (let i = 0; i < count; i++) {");
client.println("      setTimeout(() => {");
client.println("        fetch(path)");
client.println("          .then(response => console.log(`Request ${i + 1} to ${path} done`))");
client.println("          .catch(error => console.error('Error:', error));");
client.println("      }, i * delayMs);");
client.println("    }");
client.println("");
client.println("    // schedule the Mode2-off *after* the last shot");
client.println("    setTimeout(() => {");
client.println("      fetch('/Mode2/pos0')");
client.println("        .then(() => console.log('Mode 2 off request sent'))");
client.println("        .catch(err => console.error('Error sending Mode 2 off:', err));");
client.println("      setMode2Running(false);");
client.println("    }, count * delayMs);");
client.println("  });");
client.println("}");
            client.println("</script>");
client.println("<script>");
client.println("function sendRequest(path) {");
client.println("  fetch(path);");  // still send the request to the server
client.println("  document.getElementById(\"status-text\").textContent = \"ON\";");
client.println("}");
client.println("</script>");

client.println("<script>");
client.println("  let routineQueue = {};"); 
client.println("  let count = 0;");

client.println("  function addToRoutine(path) {");
client.println("    routineQueue[count] = path;");
client.println("    console.log('Added to routine:', path);");
client.println("    count++;");
client.println("  }");

client.println("  function runRoutine() {");
client.println("    let i = 0;");
client.println("    function sendNext() {");
client.println("      if (i < count) {");
client.println("        const path = routineQueue[i];");
client.println("        fetch(path)");
client.println("          .then(response => console.log('Sent:', path))");
client.println("          .catch(error => console.error('Error sending', path, error));");
client.println("        i++;");
client.println("        setTimeout(sendNext, 5000); // 5-second delay between requests");
client.println("      } else {");
client.println("        clearRoutine();");
client.println("        fetch('/Mode1/pos0')");
client.println("          .then(() => console.log('Mode 2 off request sent'))");
client.println("          .catch(err => console.error('Error sending Mode 2 off:', err));");
client.println("        setMode2Running(false);");
client.println("      }");
client.println("    }");
client.println("    // initial 10-second delay before starting the routine");
client.println("    setTimeout(sendNext, 10000);");
client.println("  }");
client.println("  function clearRoutine() {");
client.println("    routineQueue = {};");
client.println("    count = 0;");
client.println("    console.log('Routine cleared');");
client.println("  }");
client.println("</script>");
...
client.println("        <p id=\"status\">Current Mode: " + Mode +
            ...
            if(Mode == "Mode1"){
              ...
              client.println("    <button class=\"button mode2\" onclick=\"sendRequest('/Mode1/pos1')\">1</button>");
              client.println("    <button class=\"button mode2\" onclick=\"sendRequest('/Mode1/pos2')\">2</button>");
              client.println("    <button class=\"button mode2\" onclick=\"sendRequest('/Mode1/pos3')\">3</button>");

              client.println("    <button class=\"button on-off\" onclick=\"sendRequest('/Mode1/pos4')\">4</button>");
              client.println("    <button class=\"button on-off\" onclick=\"sendRequest('/Mode1/pos5')\">5</button>");
              client.println("    <button class=\"button on-off\" onclick=\"sendRequest('/Mode1/pos6')\">6</button>");

              client.println("    <button class=\"button mode2\" onclick=\"sendRequest('/Mode1/pos7')\">7</button>");
              client.println("    <button class=\"button mode2\" onclick=\"sendRequest('/Mode1/pos8')\">8</button>");
              client.println("    <button class=\"button mode2\" onclick=\"sendRequest('/Mode1/pos9')\">9</button>");

              client.println("    <button class=\"button on-off\" onclick=\"sendRequest('/Mode1/pos10')\">10</button>");
              client.println("    <button class=\"button on-off\" onclick=\"sendRequest('/Mode1/pos11')\">11</button>");
              client.println("    <button class=\"button on-off\" onclick=\"sendRequest('/Mode1/pos12')\">12</button>");




              client.println("    <button class=\"button mode2\" onclick=\"addToRoutine('/Mode1/pos1')\">1R</button>");
              client.println("    <button class=\"button mode2\" onclick=\"addToRoutine('/Mode1/pos2')\">2R</button>");
              client.println("    <button class=\"button mode2\" onclick=\"addToRoutine('/Mode1/pos3')\">3R</button>");

              client.println("    <button class=\"button on-off\" onclick=\"addToRoutine('/Mode1/pos4')\">4R</button>");
              client.println("    <button class=\"button on-off\" onclick=\"addToRoutine('/Mode1/pos5')\">5R</button>");
              client.println("    <button class=\"button on-off\" onclick=\"addToRoutine('/Mode1/pos6')\">6R</button>");

              client.println("    <button class=\"button mode2\" onclick=\"addToRoutine('/Mode1/pos7')\">7R</button>");
              client.println("    <button class=\"button mode2\" onclick=\"addToRoutine('/Mode1/pos8')\">8R</button>");
              client.println("    <button class=\"button mode2\" onclick=\"addToRoutine('/Mode1/pos9')\">9R</button>");

              client.println("    <button class=\"button on-off\" onclick=\"addToRoutine('/Mode1/pos10')\">10R</button>");
              client.println("    <button class=\"button on-off\" onclick=\"addToRoutine('/Mode1/pos11')\">11R</button>");
              client.println("    <button class=\"button on-off\" onclick=\"addToRoutine('/Mode1/pos12')\">12R</button>");

              client.println("    <button onclick=\"runRoutine()\">Run Routine</button>");
                            client.println("    <button onclick=\"clearRoutine()\">clear Routine</button>");

              //client.println("    <button onclick=\"routineQueue = []; document.querySelectorAll('.selected').forEach(btn => btn.classList.remove('selected'))\">Clear Routine</button>");
...

              ...
              //client.println("       <a href=\"/Mode1\"><button class=\"button mode1\">Mode 1 - Fixed Shot Preset</button>");
               client.println("       <a href=\"/Mode1\"><button class=\"button mode1Button\">Mode 1 - Fixed Shot Preset</button></a>");

               client.println("        <p id=\"mode1Buttons\"><b>Select Difficulty:</b></p>");
               /*
              ...
              */
                client.println("       <a href=\"/difficulty/easy\"><button class=\"button mode1Button\">Easy</button></a>");
                client.println("       <a href=\"/difficulty/medium\"><button class=\"button mode1Button\">Medium</button></a>");
                client.println("       <a href=\"/difficulty/hard\"><button class=\"button mode1Button\">Hard</button></a>");
              

 client.println("<div id=\"mode1Input\" style='margin-top: 30px;'>");
...
client.println("</div>");
              ...
              client.println("</script>");
             client.println("<script>");
client.println("function setMode2Running(isRunning) {");
client.println("  const mode1Input = document.getElementById('mode1Input');");
client.println("  const mode1Btns = document.querySelectorAll('.mode1Button');");
client.println("  const mode1Para = document.getElementById('mode1Buttons');");
client.println("  mode1Btns.forEach(btn => {");
client.println("    btn.style.display = isRunning ? 'none' : 'inline-block';");
client.println("  });");
client.println("  if (mode1Para) {");
client.println("    mode1Para.style.display = isRunning ? 'none' : 'block';");
client.println("  }");
client.println("  if (mode1Input) {");
client.println("    mode1Input.style.display = isRunning ? 'none' : 'block';");
client.println("  }");
client.println("}");
              client.println("</script>");
              ...
              client.println("       <p><button class=\"button mode1Button\" onclick=\"setMode2Running(true); sendRepeatedRequest('/Mode2running', " + String(birdieCountGlobal) + ", 5000)\">Mode 2 rep</button></p>");
